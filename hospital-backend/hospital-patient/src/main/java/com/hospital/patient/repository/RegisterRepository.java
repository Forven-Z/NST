...
/**
 * 患者域 {@code register} 读写：创建/占号/支付生命周期、患者与窗口查询（ADR-019）。
 * 叫号/结束看诊/医生队列等临床域方法见 hospital-his 同名类。
 */
@Repository
public class RegisterRepository {
    ...
    public List<Long> findIdsDueForDayClose(LocalDate today, LocalTime dayCloseTime) {
        return jdbcClient.sql("""
                        SELECT id
                        FROM register
                        WHERE delmark = 0
