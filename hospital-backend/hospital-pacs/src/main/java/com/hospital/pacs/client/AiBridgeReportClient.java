...
public class AiBridgeReportClient {
    ...
    private String extractAiReportText(Map<?, ?> body) {
        if (body == null) {
            throw new BusinessException(ErrorCode.BAD_REQUEST, "AI 报告生成服务未返回响应");
        }
        if (Boolean.FALSE.equals(body.get("success"))) {
            ...
            Object message = body.containsKey("message") ? body.get("message") : "AI 报告生成失败";
            throw new BusinessException(errorCode, String.valueOf(message));
        }
        Object data = body.get("data");
        if (!(data instanceof Map<?, ?> dataMap)) {
            throw new BusinessException(ErrorCode.BAD_REQUEST, "AI 报告生成服务响应格式无效");
        }
        Object text = dataMap.get("aiReportText");
        String aiReportText = text == null ? "" : String.valueOf(text).trim();
        if (aiReportText.isBlank()) {
            throw new BusinessException(ErrorCode.BAD_REQUEST, "AI 报告生成服务返回空诊断印象");
        }
        return aiReportText;
    }
}
