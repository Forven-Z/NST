...
public class PacsMedTechOrderCoordinator {

    private final CheckRequestRepository checkRequestRepository;

    @Transactional
    public void execute(Long checkRequestId, Long executorId) {
        int from = currentStatus(checkRequestId);
        MedTechOrderTransitions.assertTransition(from, MedTechOrderEvent.EXECUTE);
        if (checkRequestRepository.markExecutedIfCurrent(checkRequestId, from, executorId) == 0) {
