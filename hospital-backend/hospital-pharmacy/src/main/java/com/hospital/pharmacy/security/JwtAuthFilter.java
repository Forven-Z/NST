...
import com.hospital.pharmacy.config.PharmacyProperties;
...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    private final PharmacyProperties pharmacyProperties;
    private final ObjectMapper objectMapper;

    @Override
    protected boolean shouldNotFilter(HttpServletRequest request) {
        String path = request.getRequestURI();
        return path.startsWith("/internal/") || !path.startsWith("/api/v1/pharmacy/");
    }
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            Claims claims = JwtTokenHelper.parse(token, pharmacyProperties.getAuth().getJwt().getSecret());
            ...
            if (roles == null || (!roles.contains("PHARMACIST") && !roles.contains("ADMIN"))) {
                writeError(response, ErrorCode.FORBIDDEN, "需要药师角色 PHARMACIST");
                return;
            }
