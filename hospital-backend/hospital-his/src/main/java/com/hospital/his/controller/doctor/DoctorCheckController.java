...
import com.hospital.his.service.CheckOrderService;
...
@RestController
@RequestMapping("/api/v1/doctor/check-requests")
...
public class DoctorCheckController {
    ...
    public Result<Map<String, Object>> create(@Valid @RequestBody CreateInspectionRequest request) {
        return Result.success(checkOrderService.createCheckOrder(request));
    }

    @GetMapping("/{id}/result")
    public Result<Map<String, Object>> getResult(@PathVariable Long id) {
        return Result.success(checkOrderService.getCheckResult(id));
    }
}
