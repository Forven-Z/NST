...
import com.hospital.disposal.config.DisposalProperties;
...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    private final DisposalProperties disposalProperties;
    private final ObjectMapper objectMapper;

    @Override
    protected boolean shouldNotFilter(HttpServletRequest request) {
        return !request.getRequestURI().startsWith("/api/v1/disposal/");
    }

    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
            throws ServletException, IOException {
        if ("/api/v1/disposal/health".equals(request.getRequestURI())) {
        ...
        try {
            ...
            Claims claims = JwtTokenHelper.parse(token, disposalProperties.getAuth().getJwt().getSecret());
