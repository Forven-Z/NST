...
> **版本**：v2.14 | 2026-06-04 · **定稿交付**  
> **用途**：日常 **开什么、怎么开**；**联调手工验收**（§十二）。
...
| 文件 | 作用 | 不会启动 |
|------|------|----------|
| [scripts/start-project.ps1](../scripts/start-project.ps1) | **一键启动**：Java 全栈 + PC 前端 | PostgreSQL、**Python hospital-ai（8000）**、小程序 |
| [scripts/stop-project.ps1](../scripts/stop-project.ps1) | 停止 Java + 前端 | Nacos、MinIO、Python、PG |
| [scripts/start-hospital-ai.ps1](../scripts/start-hospital-ai.ps1) | **仅启 Python CNN** `:8000`（uvicorn，日志 `logs/hospital-ai/`） | Java / 前端 |
| [scripts/stop-hospital-ai.ps1](../scripts/stop-hospital-ai.ps1) | **仅停** `:8000` | Java / 前端 |
| [scripts/install-model-weights.ps1](../scripts/install-model-weights.ps1) | 将 `shared/model-weights/*.pth` 复制到 `hospital-ai/model/weights/` | 不启任何进程 |
| [scripts/render-tech-architecture.py](../scripts/render-tech-architecture.py) | 生成答辩架构图 `docs/images/tech-architecture.png` | 不启任何进程 |
| [scripts/env-cloud.ps1](../scripts/env-cloud.ps1) | 云端 `DB_HOST` / `MINIO_*` | 不启任何进程 |
| [scripts/env-local.ps1](../scripts/env-local.ps1) | 本机 `DB_HOST` / `MINIO_*` | 不启任何进程 |
...
| Nacos | `start-project` 本机启 | 同上 |

#### Python 影像 AI（P4 · 单独启）

`start-project` **不含** Python CNN。**MinIO** 已由 `start-project` 在 `local` 模式下自动启动（`:9001`），**无需**单独的 MinIO bat 脚本。

**首次本机 CNN 演示前（每人执行一次）**：

```powershell
# 1. Python 环境（若尚无 .venv，见 AI_CNN_INTEGRATION §11 / DEV_ENV_SETUP §8.3）
cd hospital-ai
python -m venv .venv
.\.venv\Scripts\activate
# 按 GPU 安装 torch 后再：pip install -r requirements.txt
cd ..

# 2. 安装 CNN 权重（git pull 后若权重有更新也需重跑）
powershell -ExecutionPolicy Bypass -File .\scripts\install-model-weights.ps1

# 3. 验证权重已就位
dir hospital-ai\model\weights\*.pth
```

**日常启动 CNN**（另开终端）：

```powershell
.\scripts\start-project.ps1              # Java 全栈（含 pacs :9104、MinIO、ai-bridge :9106）
.\scripts\start-hospital-ai.ps1          # Python CNN :8000
# 停止 CNN：.\scripts\stop-hospital-ai.ps1
# 健康检查：http://127.0.0.1:8000/v1/health（modelLoaded / lungModelLoaded / tumorModelLoaded 应为 true）
```

详见 [AI_CNN_INTEGRATION.md §11](./AI_CNN_INTEGRATION.md)。

#### 演示数据（SQL，非脚本）

| 资源 | 作用 |
|------|------|
| `docs/sql/seed-demo-check.sql` | 影像演示：`check_request` #62001 / #62002 / #62006 |
| `docs/sql/seed-demo-patients.sql` | 演示患者 + 今日内科挂号（小程序联调） |
| [DEMO_MEDICAL_RECORD_SAMPLES.md](./DEMO_MEDICAL_RECORD_SAMPLES.md) | 医生站病历/AI 诊断示例文案 |
| `docs/sql/seed-dict.sql` §药品/医技 | 20 种药品 + 34 项医技 |
...
1. 启动全栈：`.\scripts\start-project.ps1`（Nacos + 全部 Java + 可选 PC 前端）
2. 小程序目录复制联调配置：`copy hospital-patient-miniapp\config.local.example.js hospital-patient-miniapp\config.local.js`（`USE_MOCK: false`）
3. 打开 **微信开发者工具**，导入 **`hospital-patient-miniapp/`**
4. **详情 → 本地设置** → 勾选「不校验合法域名」
5. 登录页填写本人档案 → 登录；验证挂号、待缴、报告
...
| 组件 | 停法 |
| --- | --- |
| Java 服务 | IDEA Stop，或 `.\scripts\stop-project.ps1` |
...
> 答辩/demo 前自检（手工走查，经 Gateway 9000）。

### 12.1 环境准备（每次联调前）

- [ ] **环境 profile**：云端 `. .\scripts\env-cloud.ps1` 或 `start-project.ps1 -EnvProfile cloud`；本地默认 `local` 或 `. .\scripts\env-local.ps1`
...
**通过标准**：A～D 全部经 Gateway 9000 返回 `code=200`。
...
| 场景 | 要点 |
|------|------|
| 检查闭环 | 医生开检查 → 缴费 → PACS 执行/录入 → 医生可见结果 |
| 处方发药 | 医生开方 → 缴费 → 药房发药 |
| 退号/退费/退药 | 收费员退费、窗口退号、药房退药 |

### 12.5 R-full — P4（AI 演示）

- [ ] `DASHSCOPE_API_KEY` 已配置（LLM/RAG）；pgvector 已启用（见 [RAG_GUIDE.md](./RAG_GUIDE.md)）
- [ ] `install-model-weights.ps1` + `start-hospital-ai.ps1`；`/v1/health` 三模型 loaded
- [ ] 小程序 `triage/chat` 或医生站 AI 诊断/草稿可演示
- [ ] pacs 影像 AI 工作台 #62001/#62002/#62006 CNN 演示
- [ ] 停 `hospital-ai` / `ai-bridge` 后门诊主链仍可用（故障隔离）
...
| v2.17 | 2026-06-04 | 定稿交付：§12.5 R-full 改为 AI 演示 checklist；全文档同步 PROGRESS |
| v2.15 | 2026-06-04 | 恢复 `install-model-weights.ps1`；§零 补充 CNN 首次权重安装步骤 |
| v2.14 | 2026-06-04 | 精简验收脚本；保留 start/stop-hospital-ai |
