...
| 事项 | 说明 |
|------|------|
| taskType 分发 | 已完成：检查项目「胸部 CT」→ `CT_LUNG` → `LUNG_CT_ARTIFACT` |
...
老师/东软没有标注好的肺部伪影库，**不代表做不了**——用 **LIDC-IDRI 公开胸部 CT** 自标掩码（详见 `6.3/BrainCT-Lung/docs/胸部CT数据获取.md`）。

> **更正**：`C:\Neuedu\CQ500_orig` 是 **头部 CT**（491 例颅脑 CT），**不能用于肺部训练**。`data.zip` / `dicom2d` 也是头部 40 例训练数据。

| 优先级 | 来源 | 说明 |
|--------|------|------|
| **P0（推荐）** | **LIDC-IDRI（TCIA）** | 1010 例**胸部** CT，NBIA 只下 10～15 例即可（约几 GB，非全库 125GB） |
...
**落地步骤（有胸部 CT 后）** — 完整操作见 `6.3/BrainCT-Lung/docs/胸部CT数据获取.md`

```text
1. TCIA 下载 10～15 例 LIDC-IDRI 胸部 DICOM
2. 筛含金属高亮/条纹伪影的序列
...
> 临床侧无现成标注库；计划采用胸部 CT（医院脱敏或公开胸 CT 库）按与头部相同的 HU 规范自标注金属伪影掩码。集成 taskType 已通，无权重阶段 STUB 明确失败。
...
**重要**：公开数据集几乎**没有**「金属伪影掩码」 gold standard，**无法直接下载就能训**。必须 **胸部 CT + 人工掩码** 成对数据。

### 阶段 3 — 有数据后

1. 目标规模：**先 15～20 例** 胸部病例，能配成 **≥500 对** 有效切片即可开训（与头部起步类似）。
2. 目录：
...
> 肺部 CT 金属伪影任务需要 **成对的 CT 与伪影掩码**（约 15～20 例胸部 CT，含金属植入或牙科等高亮伪影）。公开库没有现成标注，希望提供脱敏 DICOM 或指导从哪个病例库筛选。集成与 taskType 已就绪，有数据后 1～2 周可出第一版 `lung_best.pth`。
