...
> **中文产品名**：智慧云脑诊疗平台  
> **文档体系版本**：v2.0 | 2026-05  
> **实施策略**：自启动 **微服务**（8×Java + 1×Python FastAPI）；业务三系统 **HIS / LIS / PACS** 各一 jar。
...
| **L2 契约** | [API.md](./API.md) | HTTP 路径与报文 |
| **L2 契约** | [DATABASE_DESIGN.md](./DATABASE_DESIGN.md) | 表结构、状态枚举 |
| **L2 契约** | [MICROSERVICES.md](./MICROSERVICES.md) | 微服务边界、路由、**架构图 §八**、M1～M10 |
| **L2 契约** | [FRONTEND_API_MAP.md](./FRONTEND_API_MAP.md) | 页面 ↔ API |
...
> **已合并删除**（勿再引用）：`INTEGRATION_CHECKLIST.md` → RUNBOOK §十二；`DEVELOPMENT_RATIONALE.md` → IMPLEMENTATION_PLAN §八；`TECH_ARCHITECTURE.md` → MICROSERVICES §八。
...
| **前端** | TEAM_COLLABORATION → FRONTEND_API_MAP → API §〇 → RUNBOOK §十二 |
