...
| ADR-018 | HIS 领域设计模式重构 | **进行中** | 就诊 **State** → 处方 **Strategy** → 医技开立 **Template Method**；详见 [REFACTORING_DESIGN_PATTERNS.md](./REFACTORING_DESIGN_PATTERNS.md) |
...
| **步骤 ④** | `AbstractMedTechExecuteTemplate` | 单模板 · LIS/PACS/Disposal 三子类 |
| **处方库存** | 开立预扣 `stock_qty`；退费/驳回/退药回增；不足拒开 | 与 SM2 同事务；见 REFACTORING §4.3.1 |
| **步骤 ⑧** | 拆 patient / pharmacy / clinical | ADR-019（模式重构完成后） |
...
- 微服务边界：**暂不变** → `MICROSERVICES.md` §2.3；待 ADR-018 实施完成后再开 ADR 修订「his 三拆」
