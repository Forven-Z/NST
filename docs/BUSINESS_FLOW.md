...
> **退费边界（与 `RefundService` 一致）**：**已缴费（status=20）** 可窗口/患者端退费；医技 **执行中（30）** 或 **已出结果（40）** 后 **不可退费**（LIS `execute` 后即进入 30）。
...
结构与 **§8.2 检查** 相同，执行角色为检验科室。退费边界同 §8.2：**已缴费（20）可退；执行中（30）/ 已出结果（40）不可退**。
